package com.work.section03.entity;
...
//    newMenu : 동일한 값을 가지지만 직접 생성한 새 객체 EntityManager가 관리하지 않는 상태 -> 비영속 상태
...
//    조회에 사용된 EntityManager 얻어오기
  ...
  void testManagedOtherEntityManager(int menuCode) {
    //when
    //lifeCycle.findMenuByMenuCode() 호출시 마다 새로운 엔티티가 생성됨 -> 서로 독립된 영속성 컨텍스트를 관리
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    // (만약에)영속 상태
    // -> tbl_menu에서 menu_code 가 일치하는 행의 price가 수정됨
    // (더티 체킹) : @Entity 객체의 변화(DB와 스냅샷 차이)를 감지)


    // (현재)준영속 상태 -> DB 테이블과 매핑 X, 자바 객체의 값만 변경됨
    // 더티 체킹X
    ...
    */
    entityManager.flush();    //DB에 전달 (commit/rollback 가능)
  ...
  void testMergeUpdate(int menuCode, String menuName) {
    ...
    Menu mergeMenu = entityManager.merge(foundMenu);//준영속 객체와 1차 캐시 내용을 병합후 영속 객체를 반환
    transaction.commit();
